...
class SubmapFitnessEvalution {
 public:
  struct Config {
    ...
    friend inline std::ostream& operator<<(std::ostream& s, const Config& v) {
      s << std::endl
        ...
        << "  k_label_overlap: " << v.k_label_overlap << std::endl
  ...
  static Config getConfigFromRosParam(const ros::NodeHandle& nh_private) {
    ...
    nh_private.param("k_label_overlap", config.k_label_overlap, config.k_label_overlap);
  ...
  std::pair<bool, float> evaluateFitnessPoints(const VoxgraphSubmap& submap_a,
    ...
    for (auto const& pt : registration_points_a.getItem()) {
      ...
      if(checkLabelValid(submap_b.getTsdfMap().getTsdfLayer(), T_B_APt[0], pt))
        num_label_valid++;
    }
    ...
    float fitness_label_0 = static_cast<float>(num_label_valid) / total_points;
    bool label_check = true;
    std::cout << "*************** label eval: " << fitness_label_0 << std::endl;

    if(fitness_label_0 < config_.k_label_overlap) {
      LOG(WARNING) << "point label eval failed: " << fitness_label_0;
      label_check = false;
    }
    if (fitness_0 < config_.k_overlap || !label_check) {
    ...
    for (auto const& pt : registration_points_b.getItem()) {
      ...
      if(checkLabelValid(submap_a.getTsdfMap().getTsdfLayer(), T_B_APt[0], pt)) {
        num_label_valid++;
      }
      if (checkDistanceValid(submap_a.getTsdfMap().getTsdfLayer(), T_B_APt[0],
                             pt.distance, config_.max_valid_distance))
    ...
    float fitness_label_1 = static_cast<float>(num_label_valid) /total_points;
    std::cout << "*************** label reverse eval: " << fitness_label_1 << std::endl;
    bool reverse_label_check = fitness_label_1 > config_.k_label_overlap;

    if (fitness_1 > config_.k_overlap && reverse_label_check) {
  ...
  bool checkLabelValid(const voxblox::Layer<voxblox::TsdfVoxel>& tsdf_layer,
                        voxblox::Point point, const RegistrationPoint &pt) {
    auto tsdf_voxel = tsdf_layer.getVoxelPtrByCoordinates(point);
    if(!tsdf_voxel) return false;
    if(tsdf_voxel->semantic_label != pt.label) return false;
    return true;
  }
