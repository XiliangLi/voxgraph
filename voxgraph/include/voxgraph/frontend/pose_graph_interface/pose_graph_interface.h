...
class PoseGraphInterface {
 public:
  ...
  bool checkLoopClosureCandidates() {
    ...
    LOG(INFO) << "*** start check loopclosure candidates ***";
  ...
  bool setPoseGraphNodeConstant(const voxgraph::SubmapNode::SubmapId &submap_id, 
                                  bool constant = true) {
    // if(constant == false) {
    //   if(pose_graph_.nodeIsConstant(submap_id)) std::cout << "=========================node is constant================" << std::endl;
    //   else std::cout << " ============================= node isn't constant =======================" << std::endl; 
    // }

    if(hasSubmapNode(submap_id)) {
      pose_graph_.setSubmapNodeConstant(submap_id, constant);
      // ROS_INFO_STREAM("set node ** " << submap_id << " ** to constant");
    } else {
      pose_graph_.setSubmapNodeConstant(submap_collection_ptr_->getFirstSubmapId(), 
                                          constant);
      ROS_INFO("can't find the middle node, set the first node to constant");
    }
    return true;
  }
